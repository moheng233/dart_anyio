...
class PerformanceMonitor {
  ...
  void _updateStats(String deviceId, PerformanceMetric metric) {
    final stats = _deviceStats.putIfAbsent(
      deviceId,
      () => ChannelPerformanceStats(deviceId: deviceId),
    );
    
    stats._updateWith(metric);
  }
}
...
class ChannelPerformanceStats {
  ...
  double get successRate => _totalOperations > 0 ? _successfulOperations / _totalOperations : 0.0;
  
  /// Average operation duration
  Duration get averageDuration => 
      _totalOperations > 0 ? Duration(microseconds: _totalDuration.inMicroseconds ~/ _totalOperations) : Duration.zero;
  
  /// Minimum operation duration
  Duration get minDuration => _minDuration == Duration(days: 1) ? Duration.zero : _minDuration;
  ...
  void _updateWith(PerformanceMetric metric) {
    ...
    final opStats = _operationStats.putIfAbsent(
      metric.operationType,
      () => _OperationStats(),
    );
    opStats._updateWith(metric);
  }
...
class _OperationStats {
  ...
  Duration get averageDuration => count > 0 ? Duration(microseconds: totalDuration.inMicroseconds ~/ count) : Duration.zero;
  Duration get actualMinDuration => minDuration == Duration(days: 1) ? Duration.zero : minDuration;
...
class PerformanceTimer {
  ...
  void finish({required bool success, Map<String, dynamic>? additionalDetails}) {
...
extension PerformanceMonitorExtension on PerformanceMonitor {
  ...
  PerformanceTimer startWriteTimer(String deviceId, {Map<String, dynamic>? details}) {
    return PerformanceTimer(
      deviceId: deviceId,
